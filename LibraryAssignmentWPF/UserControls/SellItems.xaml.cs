...
{   
    public partial class SellItems : UserControl
    {
        ...
        private void BtnDiscount_ButtonClickEvent(object sender, RoutedEventArgs e) // activate club discount
        {
            ...
            else
            {
                try
                {
                    ...
                    MessageBox.Show($"10% Club Discount activated! \nCustomer's ID: {clubDiscount.txtInput.Text}", "Discount Activated", MessageBoxButton.OK, MessageBoxImage.Information);
        ...
        private void BtnRemove_ButtonClickEvent(object sender, RoutedEventArgs e) // remove an item from the cart
        {
            if (listBox.SelectedItem != null)
            {
                ...
                MessageBox.Show($"{item.Name} has been removed from the cart", "Item Removed", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            else
            {
                MessageBox.Show("Please select an item before clicking on remove!", "ERROR", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }

        private void Button_Click(object sender, RoutedEventArgs e)
        {
            if (sender == btnAdd) // add an item to the cart
            {
                if (listBox.SelectedItem != null)
                {
                    try
                    {
                        ...
                        listBox.ItemsSource= null;
                        listBox.ItemsSource = DataBase.GetItems();
                        MessageBox.Show($"{item.Name} has been added to the cart", "Item Added", MessageBoxButton.OK, MessageBoxImage.Information);
                    }
                ...
                {
                    MessageBox.Show("Please select an item before clicking on add!", "ERROR", MessageBoxButton.OK, MessageBoxImage.Error);
                }

            }
            else if (sender == btnCart) // proceed to cart
            {
                if (itemCart.Count == 0)
                {
        ...
        private void Button_ClickCheckOut(object sender, RoutedEventArgs e) // proceed to checkout menu
        {
            double totalPrice = GetCartTotalPrice();
            MessageBoxResult result = MessageBox.Show("Are you sure you want to checkout?", "Checkout", MessageBoxButton.YesNo, MessageBoxImage.Question);
        ...
        private void Button_ClickPay(object sender, RoutedEventArgs e) // pay for the items
        {
            double totalPrice = GetCartTotalPrice();
            if (HasActivatedDiscount) totalPrice *= 0.9;
            ...
            if (amountToPay.txtInput.Text == totalPriceFloat.ToString())
            {
                try
                {
                    ...
                    SaveItemInformation();                  
                    MessageBox.Show("Thank you for your purchase! \nThe receipt has been saved in the Data Base and can be accessed by the manager.", "Purchase", MessageBoxButton.OK, MessageBoxImage.Information);
                    ReturnToWorkerMenu();
                }
        ...
        private void ReturnToWorkerMenu() // return to worker menu from last menu
        {
            ReturnToWorkerMenuFromStart();
        ...
        private void ReturnToWorkerMenuFromStart() // return to worker menu from first menu
        {
